...
namespace Matroska.Muxer.OggOpus
{
    ...
    {
        // CRC table for checksum calculating according to OGG specification
        ...
        public static uint CalculateCRC(uint CRC, ReadOnlySpan<byte> oggHeader, ReadOnlySpan<byte> data)
        {
            for (int i = 0; i < oggHeader.Length; i++)
            {
                CRC = CalculateCRC(CRC, oggHeader[i]);
            }
        ...
        private static uint CalculateCRC(uint CRC, byte b)
        {
            return (CRC << 8) ^ CRC_TABLE[((CRC >> 24) & 0xFF) ^ b];
        }
    }
}